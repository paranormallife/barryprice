...
function thmlv_shortcode_banner($atts, $content = NULL) {
	...
	if (strlen($custom_link) > 0) {
		...
		if ($link_type === 'banner_link') {
			$banner_link_open_output = '<a href="' . esc_url($banner_link['url']) . '" class="thmlv-banner-link thmlv-banner-link-full' . $link_class . '" target="'.$banner_link['target'].'">';
			$banner_link_close_output = '</a>';
		} else {
			$content_output = '<a href="' . esc_url($banner_link['url']) . '" class="thmlv-banner-link' . $link_class . '" target="'.$banner_link['target'].'">' . $content_output . '</a>';
		}
	}
