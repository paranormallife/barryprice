...
(function ($, document, window) {
	...
	publicMethod.position = function (speed, loadedCallback) {
		...
		// JIG fix: $('html').position().top
		$box.css({top: offset.top-$('html').position().top, left: offset.left, visibility:'visible'});
	...
	publicMethod.prep = function (object) {
		...
		callback = function () {
			...
			if (settings.get('transition') === 'fade') {
				$box.fadeTo(speed, 1, "JIGswing", removeFilter);
			} else {
				removeFilter();
			}
		};
		...
		if (settings.get('transition') === 'fade') {
			$box.fadeTo(speed, 0, "JIGswing", function () {
	...
	publicMethod.close = function () {
		if (open && !closing) {
			...
			$window.off('.' + prefix);
			$overlay.fadeTo(settings.get('fadeOut') || 0, 0, "JIGswing");

			$box.stop().fadeTo(settings.get('fadeOut') || 0, 0, "JIGswing", function () {
				$box.hide();
				$overlay.hide();
				trigger(event_purge);
				$loaded.remove();
