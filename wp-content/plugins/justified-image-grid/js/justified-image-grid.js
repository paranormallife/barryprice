...
function loadJustifiedImageGrid($) {
	$.justifiedImageGrid = function(element, options){
		...
		oldIE = 'opacity' in $.support && !$.support.opacity,
		anyIE = '',
					...
					for(var i = 0, j = s.selectedItems.length; i < j; i += 1){
						if(s.selectedItems[i].filters){
							for(var k = 0, l = s.selectedItems[i].filters.length; k < l; k += 1){
								...
								if(typeof s.filteredImages[s.selectedItems[i].filters[k][0]] !== 'undefined' && typeof s.filteredImages[s.selectedItems[i].filters[k][0]].push == 'function'){
		...
		var createImageElement = function(item, rowlength, id){
			...
			var link = $('<a ' + linkClass + linkRel + s.linkAttribute + titleFragment + (item.wh ? 'data-wh="'+item.wh+'" ' : '') + 'href="' + (s.lightbox != "links-off" ? href : "#") + '" />'),
