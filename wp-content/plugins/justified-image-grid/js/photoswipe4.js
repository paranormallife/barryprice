...
(function (root, factory) {
	if (typeof define === 'function' && define.amd) {
		define(factory);
	} else if (typeof module !== 'undefined' && typeof exports === 'object') { // JIG addition of extra clause
...
var _isOpen,
	...
	_unbindEvents = function() {
		...
		clearTimeout(_orientationChangeTimeout2);
	...
	init: function() {
		...
		_globalEventHandlers = {
			...
			orientationchange: function() {
				...
				// JIG adds a secondary backup thing here, just in case -.-
				clearTimeout(_orientationChangeTimeout2);
				_orientationChangeTimeout2 = setTimeout(function() {
					if(_viewportSize.x !== self.scrollWrap.clientWidth) {
						self.updateSize();
					}
				}, 1000);
		...
		// JIG addition to disable magnifier icon when zoom is disabled
		if(_options.maxSpreadZoom === 1 && _options.pinchToClose === false && _options.zoomEl === false){
			rootClasses += ' pswp--zoom-prevented';
		}
		// End of JIG addition
...
var _historyUpdateTimeout,
	...
	_parseItemIndexFromURL = function() {
		...
		if(hashMatches !== null){
			...
			jQuery.each(items, function(index,item) {
				if (typeof item.contentID !== 'undefined' && item.contentID == 'jig-contentID-'+hashTag) {
					params.pid = index;
					return false;
				}else if(item.src == hashTag){
					params.pid = index;
					return false;
				}
			});
	...
	_updateHash = function() {
		...
		/* JIG mod */
		var newHash = '!'+_options.galleryUID+'/';
		if(typeof items[_currentItemIndex].contentID !== 'undefined'){
			var contentIDregex = /(?:jig-contentID)-(\w{2})-([^\s]*)/im,
				contentIDmatches = contentIDregex.exec(items[_currentItemIndex].contentID);
			if (contentIDmatches !== null) {
				newHash += contentIDmatches[1]+"/"+contentIDmatches[2];
			}

		}else{
			newHash += items[_currentItemIndex].src;
		}
		/* /JIG mod */
		...
		if(!_historyChanged) {
			if(_windowLoc.hash.indexOf(newHash) === -1) {
				...
				// This protects against a major bug
				if(_windowLoc.hash.indexOf('%5B') === -1 && _windowLoc.hash.indexOf('%5D') === -1){
 ...
 function(pswp, framework) {
	...
	var _onControlsTap = function(e) {
		...
		_updateShareURLs = function() {
			var shareButtonOut = '',
				shareButtons = pswp.options.shareButtons,
				shareButtonData,
				shareURL,
				URI,
				description;
			...
			if(typeof pswp.options.socialButtons !== 'undefined'){
				var socialButtons = pswp.options.socialButtons;
					shareButtons = [];
					
				socialButtons = socialButtons.toLowerCase();
				socialButtons = socialButtons.split("");

				var facebookIndex = jQuery.inArray('f',socialButtons),
					twitterIndex = jQuery.inArray('t',socialButtons),
					pinterestIndex = jQuery.inArray('p',socialButtons),
					googleIndex = jQuery.inArray('g',socialButtons);

				if (~facebookIndex)	shareButtons[facebookIndex] = pswp.options.shareButtons[0];
				if (~twitterIndex) shareButtons[twitterIndex] = pswp.options.shareButtons[1];
				if (~pinterestIndex) shareButtons[pinterestIndex] = pswp.options.shareButtons[2];
				if (~googleIndex) shareButtons[googleIndex] = pswp.options.shareButtons[3];
				if(typeof pswp.options.shareButtons[4] !== "undefined"){
					shareButtons.push(pswp.options.shareButtons[4]);
				}
			}

				
			jQuery.each(shareButtons, function(i,shareButtonData){
				if(typeof shareButtonData !== 'undefined'){
					description = ((pswp.currItem.title || '')+(pswp.currItem.description ? (pswp.currItem.title ? ' ' : '')+pswp.currItem.description : '')) || document.title;
					shareURL = shareButtonData.url.replace('{{url}}', encodeURIComponent(URI) )
								.replace("{{facebook_url}}",encodeURIComponent(URI.replace("#!", (URI.indexOf('?') === -1 ? "?" : "&")+'_escaped_fragment_=')))
								.replace('{{image_url}}', encodeURIComponent(pswp.currItem.src || '') )
								.replace('{{raw_image_url}}', pswp.currItem.src || '' )
								.replace('{{title}}', encodeURIComponent(pswp.currItem.title || document.title) )
								.replace('{{description}}', encodeURIComponent(jQuery('<p>'+description.replace(/<a(?:.+?)href=['"](.+?)['"](?:.+?>)(.+?)(?:<\/a>)/mg, "$2: $1")+'</p>').text()));
	...
	// [MODIFIED FOR JIG] fix for checking both title and description
	ui.update = function() {
		...
		if(_controlsVisible && pswp.currItem) {
			...
			if(_options.captionEl) {
				...
				_togglePswpClass(_captionContainer, 'caption--empty', !pswp.currItem.title && !pswp.currItem.description);
			}
...
/*! JIGphotoSwipe(); jQuery wrapper made exclusively for JIG v3.7 by Firsh
